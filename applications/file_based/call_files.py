...
def main(path="osdt_examples/sysfiles/argtest3.yaml", key="system" ):  ##.ball.__name__, read_file=""
    ...
    vehicle = load_call(path,"dubins_vehicle", state=dubins_vehicle.State(
        x_position=0.0,y_position=0.0,orientation=0.0))

    controller = load_call(path,"dubins_controller",state=dubins_controller.State(velocity=0.75,turn=1.0))
...
def main5(path="osdt_examples/sysfiles/argtest3.yaml", key="system" ):  ##.ball.__name__, read_file=""
    ...
    heater_sys = load_call(path,"heater_system")

    controller_sys = load_call(path, "heater_controller")

    heater_sensor = load_call(path, "sensor", input_sys=heater_sys, id="heater_sensor")

    controller_sensor = load_call(path, "sensor", input_sys=controller_sys, id="controller_sensor")
...
def main2(path="osdt_examples/sysfiles/argtest3.yaml",
    ...
    ball_sys = osdt.load_call(path, key)  # ,params=p)

    pos_sensor = load_call(path, "sensor", params=sensor.Params(
        sample_field="y_position"), input_sys=ball_sys, id="pos_sensor")

    vel_sensor = load_call(path, "sensor", params=sensor.Params(
